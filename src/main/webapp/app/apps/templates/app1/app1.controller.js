(function() {
  ...
  angular
  .module('smallgisApp')
  .controller('app1Controller', app1Controller);

  app1Controller.$inject = ['$scope','geoOperation','leafletData','$http','$rootScope','Auth','$stateParams','$compile','Principal'];
  function app1Controller($scope,geoOperation,leafletData,$http,$rootScope,Auth,$stateParams,$compile,Principal) {
    ...
    function showMap(arrLayers){
      ...
      if(arrLayers.length==cont){
        ...
        leafletData.getMap("mapabase1").then(function(map) {
            ...
            for(var j=0;j<capas.length;j++){
              ...
              if(capaId==layerId){
                ...
                for(var i=0;i<features.length;i++){
                  ...
                  aData.push(feat);
                  if(geoOperation.parseNumber(attrValue)!=null){
                    sql="SELECT * FROM ? WHERE "+attrName+"="+geoOperation.parseNumber(attrValue);
                  }else{
                    sql="SELECT * FROM ? WHERE "+attrName+"='"+attrValue+"'";
                  }
                  var res = alasql(sql,[aData]);
                  var len=res.length;
                  //console.log(len);
                  if(len==1){
                ...
                if(featuresAux.length!=0){
                  filter = turf.featureCollection(featuresAux);
                  console.log(filter);
                }

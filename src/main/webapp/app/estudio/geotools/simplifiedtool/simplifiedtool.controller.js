(function() {
  ...
  angular
  .module('smallgisApp')
  .controller('simplifiedtoolController', simplifiedtoolController);

  simplifiedtoolController.$inject = ['$scope', '$element','close','geoOperation','leafletData','$rootScope'];

  function simplifiedtoolController($scope, $element,close,geoOperation,leafletData,$rootScope){
    getConfig();
    var arrLayers=[];
    $scope.mapAux;
    $scope.clcontrol;
          ...
          if($rootScope.capas[i].enabled){
            ...
            map.eachLayer(function (layer) {
              if (layer._leaflet_id === layerId) {
                ...
                if(tipo=='Polygon'||tipo=='LineString'){
    ...
    $scope.process = function() {
      try{
        ...
        if(typeof layerName !== "undefined"){
          //Invoke executeSimplify tool function
          var fcgj = executeSimplify(layerID,tolerance);
          leafletData.getMap("mapabase").then(function(map) {
            var nombreCapa='';
            if(layerName.length>=10){
              nombreCapa='Simplify ' + layerName.substring(0, 10);//Assign new name for the layer
            }else{
              nombreCapa='Simplify ' + layerName.substring(0, layerName.length);//Assign new name for the layer
            }
    ...
    function executeSimplify(layerID,tolerance) {
      var contador = 0;
      var features = [];
      var simplifyArr = [];
      ...
      for (var i = 0; i < fc.features.length; i++) {
        contador = i;
        var simplify = turf.simplify(fc.features[i],tolerance, false);
        simplifyArr.push(simplify);
      }
      var color=geoOperation.get_random_color();
      var geojson=L.geoJson(simplifyArr, {

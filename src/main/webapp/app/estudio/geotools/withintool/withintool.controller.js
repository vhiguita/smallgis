(function() {
  ...
  angular
  .module('smallgisApp')
  .controller('withintoolController', withintoolController);

  withintoolController.$inject = ['$scope', '$element','close','geoOperation','leafletData','$rootScope'];

  function withintoolController($scope, $element,close,geoOperation,leafletData,$rootScope){
          ...
          if($rootScope.capas[i].enabled){
            ...
            map.eachLayer(function (layer) {
              if (layer._leaflet_id === layerId) {
                ...
                if(tipo=='Point'){
                  arrLayers1.push({id:layerId,name:layerName});
                }else if(tipo=='Polygon'){
                  arrLayers2.push({id:layerId,name:layerName});
                }
              }
            });
          }
        }
        $scope.layers1 =arrLayers1;
        $scope.layers2 =arrLayers2;
        $scope.mapAux=map;
      });
    }
    ...
    $scope.process = function() {
      try{
        ...
        if((typeof layerName1 !== "undefined")&&(typeof layerName2 !== "undefined")){
          //Invoke executeWithin tool function
          var fcgj = executeWithin(layerID1,layerID2);
          if(fcgj!=""){
            leafletData.getMap().then(function(map) {
              ...
              nombreCapa='Within ' + nombreCapa1+ ' '+nombreCapa2;
    ...
    function executeWithin(layerID1,layerID2) {
      var features1 = [];
      var features2 = [];
      var geojson="";
      ...
      var ptsWithin = turf.within(fca, fcb);
      if(ptsWithin.features.length!=0){
        var color=geoOperation.get_random_color();
        geojson=L.geoJson(ptsWithin, {

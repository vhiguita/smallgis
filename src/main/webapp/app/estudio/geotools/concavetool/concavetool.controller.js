(function() {
  ...
  angular
  .module('smallgisApp')
  .controller('concavetoolController', concavetoolController);

  concavetoolController.$inject = ['$scope', '$element','geoOperation','leafletData','$rootScope'];

  function concavetoolController($scope, $element,geoOperation,leafletData,$rootScope){
    ...
    $scope.process = function() {
      try{
        ...
        if(typeof layerName !== "undefined"){
          ...
          var fcgj = executeconcave(layerID,distance,unit);
          if(fcgj!=null){
            leafletData.getMap().then(function(map) {
              var nombreCapa='';
              if(layerName.length>=10){
                nombreCapa='Concave ' + layerName.substring(0, 10);//Assign new name for the layer
              }else{
                nombreCapa='Concave ' + layerName.substring(0, layerName.length);//Assign new name for the layer
              }
            ...
            alert("No se puede generar el polígono cóncavo para "+layerName+" debido a que la distancia es muy pequeña.");
          }
        }
    ...
    function executeconcave(layerID, distance,unit) {
      var features = [];
      var gjson;
      ...
      var hull = turf.concave(points, distance, unit);
      if(hull!=null){

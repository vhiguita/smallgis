...
angular
    .module('smallgisApp')
    .controller('beziertoolController', beziertoolController);

beziertoolController.$inject = ['$scope', '$element', 'close','geoOperation','leafletData','$rootScope'];

function beziertoolController($scope, $element, close, geoOperation,leafletData,$rootScope){
 ...
 function getConfig(){
  ...
  $scope.process = function() {
    try{
      ...
      var sharpness=$scope.sharpness;

      if(typeof layerName !== "undefined"){
          //Invoke executeBezier tool function
          var fcgj = executeBezier(layerID);
          leafletData.getMap("mapabase").then(function(map) {
                var nombreCapa='';
                if(layerName.length>=10){
                  nombreCapa='Bezier ' + layerName.substring(0, 10);//Assign new name for the layer
                }else{
                  nombreCapa='Bezier ' + layerName.substring(0, layerName.length);//Assign new name for the layer
                }
  ...
  function executeBezier(layerID,resolution,sharpness) {
    var contador = 0;
    var features = [];
    var result = [];
    ...
    for (var i = 0; i < fc.features.length; i++) {
        var curved = turf.bezier(fc.features[i],resolution,sharpness);

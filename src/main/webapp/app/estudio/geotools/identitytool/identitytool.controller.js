(function() {
  ...
  angular
  .module('smallgisApp')
  .controller('identitytoolController', identitytoolController);

  identitytoolController.$inject = ['$scope', '$element','close','geoOperation','leafletData','$rootScope'];

  function identitytoolController($scope, $element,close,geoOperation,leafletData,$rootScope){
    ...
    $scope.process = function() {
      try{
        ...
        if((typeof layerName1 !== "undefined")&&(typeof layerName2 !== "undefined")){
          //Invoke executeIdentity tool function
          var fcgj = executeIdentity(layerID1,layerID2);
          leafletData.getMap().then(function(map) {
            ...
            var nombreCapa='Identity ' + nombreCapa1 + ' ' + nombreCapa2;
    ...
    function executeIdentity(layerID1,layerID2) {
      var fc1 = geoOperation.getFCGeombyID(layerID1,$scope.mapAux);
      var fc2 = geoOperation.getFCGeombyID(layerID2,$scope.mapAux);

      var intersect = executeIntersect(layerID1,layerID2);
      var erase = executeErase(layerID1,layerID2);
      var conflictlist = [];
      for (var f in intersect._layers) {
        conflictlist.push(intersect._layers[f].feature);
      }
      for (var g in erase._layers) {
        conflictlist.push(erase._layers[g].feature);
      }
      ...
      function executeIntersect(layerID1,layerID2) {
        ...
        var fcgj = L.geoJson(conflictlist, {

(function() {
  ...
  angular
  .module('smallgisApp')
  .controller('mergetoolController', mergetoolController);

  mergetoolController.$inject = ['$scope', '$element','close','geoOperation','leafletData','$rootScope'];

  function mergetoolController($scope, $element,close,geoOperation,leafletData,$rootScope){
    ...
    $scope.process = function() {
      try{
        ...
        if((typeof layerName1 !== "undefined")&&(typeof layerName2 !== "undefined")){
          //Invoke executeMerge tool function
          var fcgj = executeMerge(layerID1,layerID2);
          leafletData.getMap("mapabase").then(function(map) {
            var nombreCapa='';
            var nombreCapa1='';
            var nombreCapa2='';
            ...
            nombreCapa='Merge ' + nombreCapa1+ ' '+nombreCapa2;
    ...
    //Execute merge for the layers
    function executeMerge(layerID1,layerID2) {
      var features = [];
      ...
      for (var feat1 in fc1) {
        features.push(fc1[feat1].feature);
      }
      for (var feat2 in fc2) {
        features.push(fc2[feat2].feature);
      }
      var fc = turf.featurecollection(features);
      var merged = turf.merge(fc);
      var color=geoOperation.get_random_color();
      var geojson=L.geoJson(merged, {

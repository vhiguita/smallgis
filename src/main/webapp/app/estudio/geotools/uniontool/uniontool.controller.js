(function() {
  ...
  angular
  .module('smallgisApp')
  .controller('uniontoolController', uniontoolController);

  uniontoolController.$inject = ['$scope', '$element','close','geoOperation','leafletData','$rootScope'];

  function uniontoolController($scope, $element,close,geoOperation,leafletData,$rootScope){
    ...
    $scope.process = function() {
      ...
      try{
        ...
        if((typeof layerName1 !== "undefined")&&(typeof layerName2 !== "undefined")){
          //Invoke executeUnion tool function
          var fcgj = executeUnion(layerID1,layerID2);
          leafletData.getMap("mapabase").then(function(map) {
            ...
            nombreCapa='Union ' + nombreCapa1+ ' '+nombreCapa2;
    ...
    function executeUnion(layerID1,layerID2) {
      var features1 = [];
      var features2 = [];
      var result = [];
      ...
      for (var i = 0; i < fca.features.length; i++) {
        for (var j = 0; j < fcb.features.length; j++) {
          var union = turf.union(fca.features[i],fcb.features[j]);
          result.push(union);
        }
      }

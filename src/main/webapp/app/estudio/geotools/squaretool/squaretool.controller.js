(function() {
  ...
  angular
  .module('smallgisApp')
  .controller('squaretoolController', squaretoolController);

  squaretoolController.$inject = ['$scope', '$element','geoOperation','leafletData','$rootScope'];

  function squaretoolController($scope, $element,geoOperation,leafletData,$rootScope){
    ...
    $scope.process = function() {
      try{
        ...
        if(typeof layerName !== "undefined"){
          ...
          var fcgj = executesquare(layerID);
          if(fcgj!=null){
            leafletData.getMap().then(function(map) {
              var nombreCapa='';
              if(layerName.length>=10){
                nombreCapa='Square ' + layerName.substring(0, 10);//Assign new name for the layer
              }else{
                nombreCapa='Square ' + layerName.substring(0, layerName.length);//Assign new name for the layer
              }
            ...
            alert("No se puede generar el polígono cóncavo para "+layerName+" con los parámetros seleccionados.");
          }
    ...
    function executesquare(layerID) {
      var gjson;
      var bbox;
      var features = [];
      ...
      /*if(isPolygon){
         bbox = geoOperation.getBoundingBox(features);
      }else{
         bbox = geoOperation.getBBox(features);
      }*/

...
angular
    .module('smallgisApp')
    .controller('buffertoolController', buffertoolController);

buffertoolController.$inject = ['$scope', '$element','close','geoOperation','leafletData','$rootScope'];

function buffertoolController($scope, $element,close,geoOperation,leafletData,$rootScope){
  var type='';
  ...
  function getConfigBuffer(){
   ...
   $scope.process = function() {
     try{
       ...
       if(typeof layerName !== "undefined"){
           ...
           var fcgj = executeBuffer(layerID,distance,unit);
           leafletData.getMap("mapabase").then(function(map) {
                 var nombreCapa='';
                 if(layerName.length>=10){
                   nombreCapa='Buffer ' + layerName.substring(0, 10);//Assign new name for the layer
                 }else{
                   nombreCapa='Buffer ' + layerName.substring(0, layerName.length);//Assign new name for the layer
                 }
   ...
   function executeBuffer(layerID, distance,unit) {
     var contador = 0;
     var features = [];
     ...
     for (var i = 0; i < fc.features.length; i++) {
        var b = turf.buffer(fc.features[i],distance,unit);
        buffers.push(b);
     }
     var color=geoOperation.get_random_color();
     var gjson=L.geoJson(buffers, {

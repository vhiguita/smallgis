...
angular
    .module('smallgisApp')
    .controller('destinationtoolController', destinationtoolController);

destinationtoolController.$inject = ['$scope', '$element', 'close', 'geoOperation','leafletData','$rootScope'];

function destinationtoolController($scope, $element,close, geoOperation,leafletData,$rootScope){
 ...
 function getConfig(){
  ...
  $scope.process = function() {
    try{
      ...
      var bearing=$scope.bearing;
      var units=$scope.ddlunit;

      if(typeof layerName !== "undefined"){
        //Invoke executeDestination tool function
        var fcgj = executeDestination(layerID,distance,bearing,units);
        leafletData.getMap("mapabase").then(function(map) {
          var nombreCapa='';
          if(layerName.length>=10){
            nombreCapa='Destination ' + layerName.substring(0, 10);//Assign new name for the layer
          }else{
            nombreCapa='Destination ' + layerName.substring(0, layerName.length);//Assign new name for the layer
          }
  ...
  function executeDestination(layerID,distance,bearing,units) {
    var features = [];
    var result = [];
    ...
    for (var i = 0; i < fc.features.length; i++) {
        var destination = turf.destination(fc.features[i], distance, bearing, units);
